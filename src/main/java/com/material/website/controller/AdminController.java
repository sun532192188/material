...
public class AdminController {
	...
	public String left(Integer parentId, String parentFunctionName, Model model)
			throws UnsupportedEncodingException {
		Function function =  rolefunctionService.queryFunctionById(parentId);
		if(function != null){
			model.addAttribute("parentFunctionName",function.getFunctionName());
		}
		List<FunctionDto> resultList = rolefunctionService
	...
	public String moduleFunction(Model model,HttpSession session) {
		...
		if (manager != null) {
			...
			List<RoleFunctionDto> roleFunctionList = rolefunctionService
